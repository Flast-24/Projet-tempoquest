...
from rename_level_view import RenameLevelView
...
class SelectLevelToEditView(arcade.View):
    ...
    def on_draw(self):
        ...
        arcade.draw_text("ENTRÉE: Choisir, R: Renommer, SUPPR: Supprimer, ÉCHAP: Revenir",
    ...
    def on_key_press(self, key, modifiers):
        ...
        if key == arcade.key.UP:
            ...
            if self.levels:
                level_to_rename = self.levels[self.selected_index]
                rename_view = RenameLevelView(self, level_to_rename)
                self.window.show_view(rename_view)
        elif key == arcade.key.DELETE:
            if self.levels:
                level_to_delete = self.levels[self.selected_index]
                confirm_view = ConfirmDeleteView(self, level_to_delete)
                self.window.show_view(confirm_view)
